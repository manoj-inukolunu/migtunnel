...
const contentDiv = {
    props: [], request: undefined, data() {
        ...
        fetch('/all?start=0&limit=10').then(res => res.json()).then(d => {
            console.log(d);
        ...
        prev(event) {
            ...
            if (this.pageNum > 1) {
                ...
                fetch('/all?start=' + start + '&limit=' + end).then(
                    res => res.json()).then(d => {
                    console.log(d);
                    this.contentList = d.data;
                    this.recordsTotal = d.recordsTotal;
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    'HostName': $("#tunnelSubdomain").val(),
                    'TunnelName': $("#tunnelName").val(),
                    'LocalServerPort': parseInt($("#localServerPort").val())
                }),
        ...
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr v-for="data in contentList" :key="data.Id" :id="data.Id" class="hover:bg-gray-100 cursor-pointer focus:bg-gray-300 py-3" @click=loadData>
                <td class="px-4 py-3 whitespace-nowrap">
                  <div class="text-sm text-gray-900"> {{data.TunnelId}}</div>
                  <div class="text-sm text-gray-500">Time: {{data.Id}}</div>
                </td>
              </tr>
